 ...
 */
 ...
 main(int argc, char *argv[])
 {
     ...
     if(argc <= 2) {
         fprintf(stderr, "Usage: %s <window_id> <output_filename.png>\n", argv[0]);
         exit(1);
     }
     ...
     cairo_surface_t *icon = get_net_wm_icon(xid, 64);
     if (icon != NULL) {
         cairo_surface_write_to_png(icon, filename);
         cairo_surface_destroy(icon);
         fprintf(stderr, "Found _NET_WM_ICON icon, saved to %s\n", filename);
         exit(0);
     }
 
     icon = get_wm_hints_icon(xid);
     if (icon != NULL) {
         cairo_surface_write_to_png(icon, filename);
         cairo_surface_destroy(icon);
         fprintf(stderr, "Found WM_HINTS icon, saved to %s\n", filename);
         exit(0);
     }
 
     fprintf(stderr, "No icon found for window %ld\n", (long)xid);
     exit(2);
 }